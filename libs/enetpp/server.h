...
namespace enetpp {
	...
	public:
        ...
        void send_packet_to_all(enet_uint8 channel_id, const enet_uint8* data, size_t data_size, enet_uint32 flags) {
            assert(is_listening());
            if (_thread != nullptr) {
                std::lock_guard<std::mutex> lock(_packet_queue_mutex);
                auto packet = enet_packet_create(data, data_size, flags);
        ...
        void send_packet_to_all_others(unsigned int client_id, enet_uint8 channel_id, const enet_uint8* data,
